...
ViBe::ViBe(int num_sam, int min_match, int r, int rand_sam) {
    ...
    for(int i = 0; i < 9; i++) { c_xoff[i] = c_yoff[i] = c_off[i]; }
}
        ...
        for(int j = 0; j < img.cols; j++) {
            ...
            */
            if(matches >= num_min_matches) {
                ...
                // This pixel has regard as a background pixel, so the count of this pixel's foreground statistic set as
                // 0
            ...
            */
                ...
                // if this pixel is regarded as foreground for more than 50 times, then we regard this static area as
                // dynamic area by mistake, and Run this pixel as background one.
            ...
            if(matches >= num_min_matches) {
                ...
                // This pixel is already regarded as Background Pixel, then it has possibility of 1/φ to Run its model
                // sample's value.
